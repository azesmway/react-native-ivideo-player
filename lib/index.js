...
const ErrorIcon = require('./image/error.png');
...
export default class IVideo extends Component {
    ...
    _onError = e => {
        this.setState({ play: false, isError: true });
    ...
    render() {
        const { isFullscreen, play, isEnd, isError, loading, naturalSize } = this.state;
        const {
            source,
            width, height,
                ...
                {!loading && isError &&
                    <FloatView activeOpacity={1} onPress={this._onTouchEnd}>
                        <Touch activeOpacity={0.7} onPress={this._onPlay} style={{
                            ...
                            <Image source={ErrorIcon} style={{ width: 40, height: 40 }} />
                        </Touch>
                    </FloatView>
                }
